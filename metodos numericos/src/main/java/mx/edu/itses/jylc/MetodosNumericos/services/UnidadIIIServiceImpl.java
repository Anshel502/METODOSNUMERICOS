...
public class UnidadIIIServiceImpl implements UnidadIIIService {
    ...
    public Gauss AlgoritmoGauss(Gauss modelGauss) {
        ...
        if (Math.abs(A[n - 1][n - 1]) < 1e-12) {
            out.setSingular(true);
            out.setMensaje("Sistema con el pivote nulo");
            out.setMatrizUEscalonada(flatten(A));
            return out;
        }
    ...
    public GaussJordan AlgoritmoGaussJordan(GaussJordan modelGaussJordan) {
        if (modelGaussJordan == null) {
            return null;
        }

        Integer n = modelGaussJordan.getMN();
        var Aflat = modelGaussJordan.getMatrizA();
        var bList = modelGaussJordan.getVectorB();
    ...
    @Override
        ...
        if (modelGaussSeidel == null) {
            return out;
        }

        Integer n = modelGaussSeidel.getMN();
        var Aflat = modelGaussSeidel.getMatrizA();
        var bList = modelGaussSeidel.getVectorB();

        int maxIter = (modelGaussSeidel.getIteraciones() != null && modelGaussSeidel.getIteraciones() > 0) ? modelGaussSeidel.getIteraciones() : 50;
        double tol = (modelGaussSeidel.getTolerancia() != null && modelGaussSeidel.getTolerancia() > 0.0) ? modelGaussSeidel.getTolerancia() : 1e-6;
